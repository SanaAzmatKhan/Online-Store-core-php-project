...
if($_POST['method']=='userupdate'){
	...
	$ucontact=$_POST['usercontact'];
	$upass=$_POST['userpassword'];


	$sql="UPDATE user SET USER_NAME='$uname',USER_EMAIL='$uemail',USER_CONTACT='$ucontact',USER_PASS='$upass' WHERE USER_ID=$uid";
...
if($_POST['method']=='updatecate'){

	$c_id=$_POST['cid'];
	$c_name=$_POST['cname'];

	$sql="UPDATE category SET NAME='$c_name' where ID=$c_id";
...
if($_POST['method']=='updatepro'){

	$p_id=$_POST['proid'];
	$p_name=$_POST['proname'];
	$p_price=$_POST['proprice'];

	$sql="UPDATE products SET PRO_NAME='$p_name' , PRO_PRICE=$p_price where PRO_ID=$p_id";
...
if($_POST['method']=='updatesubcate'){

	$sc_id=$_POST['scid'];
	$sc_name=$_POST['scname'];

	$sql="UPDATE sub_category SET SUB_CAT_NAME='$sc_name' where SUB_CAT_ID=$sc_id";
...
if($_POST['method']=='OrderDone'){
	...
	$qurey55="UPDATE `order_detail` SET `Status` = '2' WHERE `order_detail`.`ORDER_ID` = '$order_ID';";
		$result55=$con->query($qurey55);
		if($result55){
	...
	$sql344="SELECT *FROM order_done where OR_ID=".$order_ID;
